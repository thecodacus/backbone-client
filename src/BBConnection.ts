...
export class BBConnection{
  ...
  private conn:BBChannel;
  private auth:BBChannel;
  ...
  runRequest(request:Array<operation>,connetable:boolean=false):Observable<BBChannel>{
    if(this.closed){throw Error("Backbone Connection Is Closed")}

    let ob=new Observable<BBChannel>((observer)=>{
    ...
    if(connetable) return ob.pipe(multicast(new Subject<BBChannel>()))
    else return ob;
  }
